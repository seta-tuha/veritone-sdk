...
import GeoSearchControl  from 'leaflet-geosearch/lib/leafletControl';
  ...
  renderMap(element) {
    if(!this.state.renderedMap) {
      if(element) {
        ...
        let map = new L.Map(element, { attributionControl: false } ).setView(position, 14);
        ...
        var drawControl = new L.Control.Draw( getMapControlOptions(drawnItems) );
